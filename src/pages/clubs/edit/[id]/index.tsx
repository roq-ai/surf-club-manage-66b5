...
import { getClubById, updateClubById } from 'apiSdk/clubs';
...
function ClubEditPage() {
  ...
  const handleSubmit = async (values: ClubInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateClubById(id, values);
      mutate(updated);
      resetForm();
      router.push('/clubs');
    } catch (error) {
  ...
  const formik = useFormik<ClubInterface>({
    initialValues: data,
